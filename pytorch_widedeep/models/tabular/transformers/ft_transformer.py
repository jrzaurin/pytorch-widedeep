...
class FTTransformer(BaseTabularModelWithAttention):
    ...
    Args:
        ...
        ff_factor (float, default = 4 / 3): Multiplicative factor applied to the first layer of the FF network in
            each Transformer block, This is normally set to 4, but they use 4/3
            in the paper.
        mlp_hidden_dims (List, Optional, default = None): MLP hidden dimensions. If not provided no MLP on top of the final
    ...
    Attributes:
        ...
        fttransformer_blks (nn.Sequential): Sequence of FTTransformer blocks
        fttransformer_mlp (nn.Module): MLP component in the model
