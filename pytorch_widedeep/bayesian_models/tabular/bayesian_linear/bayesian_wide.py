...
class BayesianWide(BaseBayesianModel):
    ...
    Args:
        input_dim (int): size of the Embedding layer. ``input_dim`` is the summation of all the
        ...
        pred_dim (int): size of the ouput tensor containing the predictions
        prior_sigma_1 (float, default = 1.0): The prior weight distribution is a scaled mixture of two Gaussian
            densities:
            .. math::
            \begin{aligned}
    ...
    Attributes:
        bayesian_wide_linear (nn.Module): the linear layer that comprises the wide branch of the model
