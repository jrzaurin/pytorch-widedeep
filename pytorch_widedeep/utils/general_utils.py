...
class Alias:
    def __init__(self, primary_name: str, aliases: Union[str, List[str]]):
        ...
        Args:
            primary_name (String): Preferred name for the parameter, the value of which will be set
            ...
            aliases (List, string): One or multiple string aliases for `primary_name`. If
...
def set_default_attr(obj: Any, name: str, value: Any):
    ...
    Args:
        obj (Object): Object whose `name` attribute will be returned (after setting it to
            `value`, if necessary)
        name (String): Name of the attribute to set to `value`, or to return
        value (Object): Default value to give to `obj.name` if the attribute does not already
            exist

    Returns:
        Object (obj.name): if it exists. Else, `value`
