                    ...
                    if inspect.isclass(f_):
                        ...
                        for limb in child.body:
                            if isinstance(limb, ast.FunctionDef):
                                if not ((child.name in exclude_class_methods_dict and limb.name in exclude_class_methods_dict[child.name]) or limb.name.startswith("_")):
...
def main():
    ...
    automate_mkdocs_from_docstring(
        mkgendocs_f="mkdocs/mkgendocs.yml",
        repo_dir=repo_dir,
        match_string="pages:\n",
    )
    ...
    try:
        shutil.rmtree("mkdocs/templates/examples")
    except OSError as e:
        print("Error: %s : %s" % ("mkdocs/templates/examples", e.strerror))
    os.makedirs("mkdocs/templates/examples")
    ...
    for file in onlyfiles:
        if ".ipynb" in file:
            shutil.copy(file, "mkdocs/templates/examples/")
