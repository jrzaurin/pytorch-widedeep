...
flash_net = (
    MultiHeadedAttention(
        input_dim=input_dim, n_heads=n_heads, use_bias=False, dropout=0, use_flash=True
    )
    .to(device)
    .eval()
)
...
for module in ["q_proj", "kv_proj", "out_proj"]:
    getattr(flash_net, module).weight = copy.deepcopy(
        getattr(standard_net, module).weight
    )
