...
@pytest.mark.parametrize('input_df, encoding_dict, output_df',
	[
...
encoding_dict_1 = {c: {k:v for v,k in enumerate(sorted(df_letters[c].unique()))} for c in df_letters.columns}
encoding_dict_2 = {c: {k:v for v,k in enumerate(sorted(df_numbers[c].unique()))} for c in df_numbers.columns}

df_letters_le, letters_enconding_dict = label_encoder(df_letters, cols=['col1', 'col2'], val_to_idx=encoding_dict_1)
df_numbers_le, numbers_enconding_dict = label_encoder(df_numbers, cols=['col1', 'col2'], val_to_idx=encoding_dict_2)
...
def test_prepare_deep_without_continous_columns(input_df, X_deep, embed_input, decoding_dict,
	error_list):
	for i,c in enumerate(input_df.columns):
		if input_df[c].nunique() != embed_input[i][1] or cat_embed_cols[i][1] != embed_input[i][2]:
			error_list.append("error: the setup output does not match the intended input")
